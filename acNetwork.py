...
class AC_Network():
    def __init__(self,s_size,a_size,scope,trainer):
        ...
        with tf.variable_scope(scope):
            ...
            self.conv1 = tf.layers.conv2d(inputs=self.imageIn, filters=16, kernel_size=[3, 3])
            self.conv2 = tf.layers.conv2d(inputs=self.conv1, filters=32, kernel_size=[3, 3])
            ...
            #     kernel_size=[3,3],stride=[1,1],padding='VALID')
            hidden = slim.fully_connected(slim.flatten(self.conv2),128,activation_fn=tf.nn.relu)
            # flatten_layer = tf.contrib.layers.flatten(hidden2)
            # dense_connected_layer = tf.contrib.layers.fully_connected(flatten_layer, 32, activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer(), biases_initializer=None)
            ...
            # self.tetromino_onehot = tf.reshape(tf.one_hot(self.tetromino,3,dtype=tf.float32), shape=[-1, 3])
            # self.tetromino_hidden = tf.layers.dense(inputs=self.tetromino_onehot, units=24, activation=tf.nn.relu)
            # self.concat_layer = tf.concat([self.tetromino_onehot, flatten_layer], 1)
            ...
            self.policy = tf.layers.dense(inputs=hidden, units=a_size, activation=tf.nn.softmax)
            self.value = tf.layers.dense(inputs=hidden, units=1)
