...
class AC_Network():
    def __init__(self,s_size,a_size,scope,trainer):
        ...
        with tf.variable_scope(scope):
            ...
            self.conv1 = tf.layers.conv2d(inputs=self.imageIn, filters=16, kernel_size=[1, 1])
            ...
            #     kernel_size=[3,3],stride=[1,1],padding='VALID')
            hidden = slim.fully_connected(slim.flatten(self.conv3),64,activation_fn=tf.nn.relu)
            ...
            self.policy = tf.layers.dense(inputs=dense_connected_layer, units=a_size, activation=tf.nn.softmax)
            self.value = tf.layers.dense(inputs=dense_connected_layer, units=1)
